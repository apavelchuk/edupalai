...
async def foo_async():
    ai = factories.ai()
    reply_stream = ai.reply_stream(
        "Hello how are you I'm fine thank you. Give me a couple of sentences as a response please."
    )
    ttv = factories.text_to_voice(stream=True)
    audio_stream = ttv.text_to_voice(ContentLanguage.ENGLISH, reply_stream)
