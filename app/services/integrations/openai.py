...
from app.logger import Logger, logger_factory
...
class RequestMaker(ABC):
    ...
    def __init__(self, auth_token: str, model_type: ModelType, logger: Logger):
        self.model: str = self.MODELS[model_type]
    ...
    def make_streaming_request(self, data: dict) -> AsyncIterator[str]:
        raise NotImplementedError()


class CompletionRequestMaker(RequestMaker):
    URL = "https://api.openai.com/v1/completions"
    MODELS = {
        ModelType.SIMPLE: "text-ada-001",
        ModelType.ADVANCED: "text-davinci-003"
    }
...
class ChatRequestMaker(RequestMaker):
    URL = "https://api.openai.com/v1/chat/completions"
    MODELS = {
        ModelType.ADVANCED: "gpt-3.5-turbo"
    }
    ...
    def get_text_from_streaming_chunk(self, text_chunk: bytes) -> Optional[str]:
        ...
        try:
            ...
            # first is empty, second will include only "role":"assistant" and third only newlines
        except Exception:
            raise ServiceException(text_chunk, self.logger)
        if not deltas:
            return None
...
def openai_service_factory() -> AI:
    logger = logger_factory("OpenAI")
    req_maker = ChatRequestMaker(
